...
LOG_FILE = "bot_DOGE.log"

# ---------------------------------------------------------
# Logging Setup
# ---------------------------------------------------------
logger = logging.getLogger("DOGE_BOT")
...
try:
    ...
    logger.info(f"🐶 [DOGE] Bot Started! (Target ROE {TARGET_ROE}%)")
...
def get_market_data():
    try:
        # Fetch 50 candles for pattern recognition
        ...
        Price vs EMA: {"ABOVE (Bullish)" if latest['close'] > latest['ema'] else "BELOW (Bearish)"}
        Volume Status: {"HIGH (Spiking)" if latest['volume'] > latest['vol_ma'] else "Normal"}
        """
